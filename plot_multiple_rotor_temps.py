...
def usage():
    print("plot_multiple_rotor_temps.py <rotor name 1> <rotor name 2> ... <rotor name N>")
...
def calculateStatistics(phaseNames, ambPhaseTemps, topPhaseTemps, botPhaseTemps, rtrPhaseTemps):
    rtrStats = []
    for phase, amb, top, bot, rtr in zip(phaseNames, ambPhaseTemps, topPhaseTemps, botPhaseTemps, rtrPhaseTemps):
        ...
        print("  Ambient Temps:")
        if len(amb) > 0:
            print("    Min: %.3f" % np.min(amb))
            print("    Max: %.3f" % np.max(amb))
            print("    Avg: %.3f" % np.mean(amb))
            print("    Std: %.3f" % np.std(amb))
            print("    # Readings: %d" % len(amb))
        else:
            print("    No temperature readings.")
        print("  Top Plate Temps:")
        if len(top) > 0:
            print("    Min: %.3f" % np.min(top))
            print("    Max: %.3f" % np.max(top))
            print("    Avg: %.3f" % np.mean(top))
            print("    Std: %.3f" % np.std(top))
            print("    # Readings: %d" % len(top))
        else:
            print("    No temperature readings.")
        print("  Bottom Plate Temps:")
        if len(bot) > 0:
            print("    Min: %.3f" % np.min(bot))
            print("    Max: %.3f" % np.max(bot))
            print("    Avg: %.3f" % np.mean(bot))
            print("    Std: %.3f" % np.std(bot))
            print("    # Readings: %d" % len(bot))
        else:
            print("    No temperature readings.")
        print("  Rotor Temps:")
        if len(rtr) > 0:
            print("    Min: %.3f" % np.min(rtr))
            print("    Max: %.3f" % np.max(rtr))
            print("    Avg: %.3f" % np.mean(rtr))
            print("    Std: %.3f" % np.std(rtr))
            print("    # Readings: %d" % len(rtr))
...
style = ['-', '--', ':', '-.']
color = ['red', 'blue', 'green', 'purple', 'orange', 'deeppink', 'black', 'gray', \
         'deepskyblue', 'lawngreen', 'magenta']
argc = len(sys.argv)
if argc < 2:
    usage()

rotor_names = sys.argv[1:]
color_count = 0
...
for rotor_name in rotor_names:
    ...
    if not os.path.exists(p_infilename):
        p = subprocess.Popen(["python3", "C:\\Users\\awong\\Documents\\Analyzer_Decoding_Files\\extract_phase_timestamps.py", rotor_name])
        p.wait()

    if "Group1Data" not in g1_infilename or "PhaseTimestampsOut" not in p_infilename:
    ...
    plt.plot(ambTempTime, ambTemp, label=rotor_name + ' Ambient Temperature', color=color[color_count], linestyle='-.')
    plt.plot(topTempTime, topTemp, label=rotor_name + ' Top Plate Temperature', color=color[color_count], linestyle='--')
    plt.plot(botTempTime, botTemp, label=rotor_name + ' Bottom Plate Temperature', color=color[color_count], linestyle=':')
    plt.plot(rtrTempTime, rtrTemp, label=rotor_name + ' Rotor Temperature', color=color[color_count], linestyle='-')
    ...
    stats = calculateStatistics(phaseNames, ambPhaseTemps, topPhaseTemps, botPhaseTemps, rtrPhaseTemps)

    allRtrTempData.append(stats)

    color_count += 1

print("\n          Rotor Name             Min Temp   Max Temp   Avg Temp    St Dev    Readings")
print("------------------------------   --------   --------   --------   --------   --------")
for i in range(len(allRtrTempData[0])):
    print("%s" % (allRtrTempData[0][i][0].upper()))
    for j in range(len(rotor_names)):
        print("%30s   %8.3f   %8.3f   %8.3f   %8.3f   %d" % (rotor_names[j], allRtrTempData[j][i][1], allRtrTempData[j][i][2], allRtrTempData[j][i][3], allRtrTempData[j][i][4], allRtrTempData[j][i][5]))
    print(" ")
plt.legend()
plt.show()